...
void run_sphere_tests()
{
	...
	vec3 positive_x_position = vec3(2 * sphere_radius, 0.0f, 0.0f);
	...
	vec3 off_center_x_position = vec3(2* sphere_radius, sphere_radius * 0.5f, 0.0f);
	...
	assert(output.front_face == true);
	assert(output.time > 1.0f);
	assert(output.position.x() < 1.0f);	// collision point is different to the test case hitting the sphere at the origin
	assert(output.position.y() == 0.0f);
	assert(output.position.z() == 0.0f);
